    ...
    const loadContent = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setContent(translate('ui.errors.contentNotFound', {}, 'Content not found'));
        }
      } catch (error) {
        setContent(translate('ui.errors.errorLoadingContent', {}, 'Error loading content'));
      } finally {
        setLoading(false);
      }
    };
        ...
        return (
          <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
            <p className="text-gray-600">
              {translate('ui.errors.moduleNotFound', 
                { module: personalizedConfig.interactiveModule }, 
                `Interactive module "${personalizedConfig.interactiveModule}" not found`
              )}
            </p>
          </div>
        );
